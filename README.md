...
This option is build-in Dockerfile which will have Apache and PHP. When running phpinfo() in /src/index2.php and if everything works should see PHP has Redis supported see below. <strong><em>Please note if runs behind a proxy we need to disable proxy during docker build this is the case inside a corporation.</em></strong>
