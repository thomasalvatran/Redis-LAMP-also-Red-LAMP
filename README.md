...
This option is build-in Dockerfile which will have Apache and PHP. When running phpinfo() in /src/index2.php and if everything works should see PHP has Redis supported see below. <em>Please note if runs inside a proxy we need to disable proxy during docker build.</em>
